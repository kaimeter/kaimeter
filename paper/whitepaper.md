...
Instead, the guest is a **fixed, versioned rule-bundle interpreter** with a stable image identifier. It takes the bundle $B$ as a witness, recomputes $h_B = H(\mathrm{canon}(B))$ **inside the proof**, checks the recomputed value against the public input $h_B$, and only then evaluates $F$ over the private inputs $w$ using the rules in $B$. The bundle travels as data; the interpreter's image ID changes only when the interpreter changes, not when a rule, a default value or a CN code changes. This gives the properties claimed:

- **Rule identity (R3).** The public $h_B$ is not merely asserted by the prover; it is recomputed in-circuit from the bundle witness, so the rules actually evaluated are the rules whose hash appears in the envelope.
...
The cost is that canonical hashing and rule dispatch are proven rather than native, and the bundle witness increases proving time and proof size. The canonical hash must be computable inside the proof system: SHA-256 over the canonical serialisation is available but expensive in most zkVMs, so the bundle may instead be committed with a proof-system-friendly hash over the same serialisation, with the two constructions documented and conformance-tested. A hand-written circuit remains a fallback where proving cost dominates; the same interpreter contract — bundle as witness, $h_B$ recomputed in-circuit, stable key — applies to it.

Two points should be stated plainly. First, **no prover exists at the time of publication**; the first interpreter guest build is planned for the second release of the reference implementation (§10). Second, based on published benchmarks for comparable workloads, proving time for a single installation's methodology at the fixed-point precision above, together with in-circuit bundle hashing, is expected to be seconds to low minutes on commodity hardware, with verification in milliseconds — neither a bottleneck for annual or per-shipment use. These expectations will be replaced by measurements in a later version of this paper.
...
A reference implementation, **Kaimeter**, is being developed as open source under Apache 2.0 at `https://github.com/kaimeter/kaimeter`. At the time of this preprint it is not yet released; the description below is a specification and a commitment, not a report of results. A later version of this paper will replace this section with the status of a tagged release.
...
| v0.3 | Q1 2027 | `sectors/steel/`; proof envelope and offline verifier; stable interpreter image IDs; attestation-policy schema and key resolution; signed bundle distribution; reproducible-build pipeline with SBOM and provenance. |
...
3. **Implementation status.** At the time of this preprint the reference implementation has not been released. The claim that the architecture is sector-agnostic rests on the bundle structure and on the observation that each sector's methodology is arithmetic over attested inputs in the same way; it has not yet been demonstrated in code for any sector. Cement and fertilisers, with indirect emissions, will be the first real test of `common/indirect.rs`.
...
6. **Provable-evaluator correctness.** No prover exists yet. When it does — as the fixed interpreter of §5.4 or a hand-written circuit — the differential test suite will be strong evidence but not proof of equivalence with the plain evaluator, and a soundness bug would be a silent failure. The in-circuit canonical-hash implementation is itself part of what must be audited. Independent audit is a prerequisite for anyone relying on proofs for financial decisions.
