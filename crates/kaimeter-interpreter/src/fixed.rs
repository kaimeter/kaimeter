...
//! of the bundle identity, so an independent evaluator — native or in circuit
//! — can reproduce every intermediate value exactly.
...
impl Fixed {
    ...
    /// Rounds to the nearest whole number, half to even.
    ///
    /// The result stays in range: rounding can move a value at most half a
    /// unit toward zero's neighbour, and the scaled range ends well before
    /// an integer multiplication could leave it.
    #[must_use]
    pub fn round_to_integer(self) -> Self {
        let quotient = self.0 / SCALE;
        let remainder = self.0 % SCALE;
        let half = SCALE.unsigned_abs() / 2;
        let magnitude = remainder.unsigned_abs();
        let rounds_up = magnitude > half || (magnitude == half && quotient % 2 != 0);
        let step = i128::from(rounds_up);
        let adjusted = match remainder.cmp(&0) {
            Ordering::Greater => quotient + step,
            Ordering::Less => quotient - step,
...
mod tests {
    ...
    #[test]
    fn rounds_to_whole_numbers_half_even() {
        assert_eq!(fixed("2.4").round_to_integer(), fixed("2"));
        assert_eq!(fixed("2.5").round_to_integer(), fixed("2"));
        assert_eq!(fixed("3.5").round_to_integer(), fixed("4"));
        assert_eq!(fixed("-2.5").round_to_integer(), fixed("-2"));
        assert_eq!(fixed("-3.5").round_to_integer(), fixed("-4"));
        assert_eq!(
            Fixed::from_scaled(i128::MAX).round_to_integer(),
            Fixed::from_scaled(170_141_183_460_469_231_731_687_303_715_884_000_000)
        );
        assert_eq!(
            Fixed::from_scaled(i128::MIN).round_to_integer(),
            Fixed::from_scaled(-170_141_183_460_469_231_731_687_303_715_884_000_000)
        );
    }
    ...
    fn takes_absolute_values() {
        assert_eq!(fixed("-0.5").try_abs().unwrap(), fixed("0.5"));
        assert_eq!(fixed("0.5").try_abs().unwrap(), fixed("0.5"));
        assert_eq!(Fixed::ZERO.try_abs().unwrap(), Fixed::ZERO);
        assert_eq!(
            Fixed::from_scaled(i128::MIN).try_abs(),
            Err(RuleError::Overflow)
        );
    }
