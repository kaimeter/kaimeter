...
//! ```
//!
//! The construction lives here, once, so that the hash recomputed inside a
//! zero-knowledge guest and the hash computed natively cannot drift
//! (whitepaper §5.4; interpreter contract §4).
...
pub const CANONICAL_PREFIX: &[u8] = b"kaimeter-bundle-v1\n";
...
\x03\x00\x00\x00\x00\x00\x00\x00{}\n";
  ...
  "appliesTo": null,
  "legalBasis": ["Regulation (EU) 2023/956"],
  "supersedes": null,
  "changelog": [{"version": "2026.1.0", "note": "first"}],
  "excludedPaths": ["tests/**"]
}"#;
        let metadata = BundleMetadata::from_json(text).unwrap();
        assert_eq!(metadata.bundle, "cbam");
        assert_eq!(metadata.version, "2026.1.0");
        ...
        assert_eq!(metadata.legal_basis.len(), 1);
        assert_eq!(metadata.supersedes, None);
        assert_eq!(metadata.changelog.len(), 1);
        assert_eq!(metadata.changelog[0].version, "2026.1.0");
        assert_eq!(metadata.changelog[0].note, "first");
        assert_eq!(metadata.excluded_paths, ["tests/**"]);
        assert!(metadata.excludes("tests/a.json"));
